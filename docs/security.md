...
| **Fake manifest** (attacker crafts a manifest, or splices real signed component entries from different manifests together) | The manifest-level signature (`manifest_signature`, `manifest.CanonicalPayload`) covers every component's name+version+sha256 as one signed unit -- a splice changes the payload and breaks the signature, even though each individual component signature might still verify on its own. This is a deliberate strengthening beyond a per-component-only signature scheme. |
