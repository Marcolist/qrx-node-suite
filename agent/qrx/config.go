...
type Config struct {
	...
	CLIPath string
	// DataDir is passed as -datadir when non-empty.
	DataDir string
	// Network selects mainnet/testnet/regtest etc, passed as -network when
	// non-empty. Never assumed -- an empty value means "let qrx-cli decide",
	// which callers should treat as Unknown rather than "mainnet".
	Network string
	// WalletName is passed as -wallet when non-empty.
	WalletName string
